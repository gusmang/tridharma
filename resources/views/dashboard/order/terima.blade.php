...
<div class="peserta">
                    ...
                    <div id="accordion">
                        ...
                        <div class="card">
                            <div class="card-header" id="heading<?php echo $rows->id_acara; ?>">
                                <h5 class="mb-0">
                                    {{-- <button class="btn btn-link" onclick="window.location = `list-terima?status=<?php //echo $_GET['status']; ?>&id_acara=<?php //echo $rows->id_acara; ?>&tanggal=${$('#t_date_picker').val()}`">
                            ...
                            <div id="collapse<?php echo $rows->id_acara; ?>" class="collapse <?php echo $no == $ktm ? "show" : ""; ?>" aria-labelledby="headingOne" data-parent="#accordion">
                                ...
                                <?php
                                    $acaras = isset($_GET['id_acara']) ? $_GET['id_acara'] : $acara_id;
                                    if ($rows->id_acara == $acaras) {
                                    ?>
                                <div>
                                    List Penerima
                                    <hr />
                                    <nav>
                                        <div class="nav nav-tabs" id="nav-tab<?php echo isset($_GET['id_acara']) ? $_GET['id_acara'] : "0"; ?>" role="tablist">
                                            <a class="nav-item nav-link <?php echo (isset($_GET['status']) && $_GET['status'] == 1) ? "active" : "";  ?>" id="nav-home-tabs" data-toggle="tab" href="#belum_bayar<?php echo isset($_GET['id_acara']) ? $_GET['id_acara'] : "0"; ?>" role="tab" aria-selected="true" style="font-size:11px; cursor:pointer;">Belum diterima</a>
                                            <a class="nav-item nav-link" id="nav-home-tab" id="nav-home-tabs2" data-toggle="tab" href="#sudah_bayar<?php echo isset($_GET['id_acara']) ? $_GET['id_acara'] : "0"; ?>" aria-selected="true" style="font-size:11px; cursor:pointer;">Sudah diterima</a>
                                        </div>
                                    </nav>
                                    <div class="tab-content" id="nav-tabContent<?php echo isset($_GET['id_acara']) ? $_GET['id_acara'] : "0"; ?>">
                                        <div class="tab-pane fade show active" id="belum_bayar<?php echo isset($_GET['id_acara']) ? $_GET['id_acara'] : "0"; ?>" role="tabpanel" aria-labelledby="nav-home-tab">
                                            <?php
                                                    foreach ($list_penerima as $rows_penerima) {
                                                    ?>
                                            <div class="col-12" style="margin-bottom:10px;">
                                                <div class="row">
                                                    <div class="col-6"><?php echo $rows_penerima->first_name . " " . $rows_penerima->last_name; ?>
                                                        <br /> Rp. {{ number_format(($rows_penerima->nominal_sub),0,'.','.') }}
                                                    </div>
                                                    <div class="col-6">
                                                        <?php
                                                                    if ($rows_penerima->nominal_sub == "0") {
                                                                    ?>
                                                        <i class="fa fa-check" style="color:green; font-size: 32px;"></i> Sudah Diterima
                                                        <?php
                                                                    } else {
                                                                    ?>
                                                        <button class="btn btn-success" style="width:100%;" onClick="confirm_terima('<?php echo $rows_penerima->id_penerima; ?>','<?php echo $rows_penerima->id_acara; ?>','<?php echo $_GET['tanggal']; ?>')"> Terima </button>
                                                        <?php
                                                                    }
                                                                    ?>
                                                    </div>
                                        ...
                                        <div class="tab-pane fade show" id="sudah_bayar<?php echo isset($_GET['id_acara']) ? $_GET['id_acara'] : "0"; ?>" role="tabpanel" aria-labelledby="nav-home-tab">
                                            <?php
                                                    foreach ($list_penerima_sudah as $rows_penerima) {
                                                    ?>
                                            <div class="col-12" style="margin-bottom:10px;">
                                                <div class="row">
                                                    <div class="col-6"><?php echo $rows_penerima->first_name . " " . $rows_penerima->last_name; ?>
                                                    ...
                                                    <div class="col-6">
                                                        <i class="fa fa-check" style="color:green; font-size: 32px;"></i>
                                                    </div>
                                                </div>
                                            </div>
                                            <?php
    ...
    <script>
        ...
        function confirm_terima(id_staff, id_acara, tanggal) {
            var conn = confirm("Terima pembayaran ini ? ");

            if (conn) {
                $.ajax({
                    type: "POST"
                    , url: "<?php echo route('terima-bayar'); ?>"
                    , data: "id_staff=" + id_staff + "&id_acara=" + id_acara + "&tanggal=" + tanggal + "&_token=<?php echo csrf_token(); ?>"
                    , dataType: "json"
                    , success: function(data) {
                        if (data.status == 200) {
                            window.location = "<?php echo url('dashboard/list-terima?status=' . $_GET['status'] . '&tanggal=' . $_GET['tanggal']); ?>"
                        } else {
        ...
        function link_tab(status) {
            ...
            Y - m - d "); ?>";
            window.location = "list-terima?status=" + status + "&tanggal=" + tanggal;
        }
        ...
        function filterDates(action) {
            ...
            window.location = "list-terima?status=1&tanggal=" + new_date;
        }
        ...
        function loadDatatable(param) {
            ...
            var url = `{{ route('list-order.dataterima') }}?${param}`;
            var config = {
                processing: true
                , serverSide: true
                , ajax: url
                ...
                , columns: [{
                    ...
                    , {
                        ...
                        , render: function(data, type, row) {
                            return `${formatRupiah(row.is_cash == 0 ?  "Rp. 0" : ((parseInt(row.sisa_pembayaran_peserta))).toString() , "Rp. ")}`;
                        }
                    }
                    , {
                        data: 'nama_penerima'
                    ...
                    , {
                        ...
                        , render: function(data, type, row) {
                            return `<div>
                                    <a href="{{ url('dashboard/list-terima/bayar') }}/${data}" class="btn btn-success">Kwitansi</a>
                                    </div>`;
                        }
                    },
                    // {
                    ...
                    //     render: function(data, type, row) {
                    //         return `<div>
                    //                 <a href="{{ url('dashboard/list-terima/bayar') }}/${data}" style="display:${row.sudah_bayar == 1 ? 'none' : 'inline-block'}" class="btn btn-success">Bayar</a>
                    ...
                    //                     <a href="{{ route('list-terima.index') }}/${data}" class="btn btn-info">Upload</a>
                    //                 </div>
                    //                 </div>`;
                    //     }
                    // },
                    ...
                    //                 <a href="{{ route('list-terima.index') }}/${data}" class="btn btn-warning">Cetak</a>
