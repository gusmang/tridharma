...
class FrontEndController extends Controller
{
    ...
    public function daftarStore(Request $request) {
        ...
        // if(!$request->all()['g-recaptcha-response']){
        //     return back()->withInput($request->all())->with('notif-warning','Tolong click checkbox iam not robot');
        // }
        // Check Acara
        $acara = Acara::whereId($request->acara_id)->first();
        if(!$acara)
            return back()->withInput($request->all())->with('notif-warning','Acara Tidak di temukan');
        ...
        if($request->jadwal) {
            ...
            if(!$jadwal)
                return back()->withInput($request->all())->with('notif-warning','Validation Failedo');
        }
        // dd(((int)$acara->punia * $request->jumlahPeserta)+$this->code());
        //check sistem_kepersertaan
        if($acara->sistem_kepesertaan =='Satu Orang' && $request->jumlahPeserta >1){
            return back()->withInput($request->all())->with('notif-warning','Jumlah Peserta Maksimum 1');
        }
        // get Number Urut;
        ...
        //get last id;
        // dd($request->all());
        // $url = "https://www.google.com/recaptcha/api/siteverify?secret=".env('RECAPTCA_SECRET_KEY')."&response=".$request->all()['g-recaptcha-response']."&remoteip=".$_SERVER['REMOTE_ADDR'];
        // $response = json_decode(file_get_contents($url), true);
        // if($response['success'] == false)
        // {
            $code = $this->code();
            $new_punia =( (int)$acara->punia * (int)$request->jumlahPeserta)+$code;
